...
var SequelizeField = (function () {
  ...
  function SequelizeField(_ref) {
    ...
    this._setFileHook = function (instance, options) {
      var file = instance.getDataValue(_this._VIRTUAL_ATTRIBUTE_NAME);

      if (instance.previous(_this._PATH_ATTRIBUTE_NAME) === file || typeof file === 'undefined') return;

      if (_this._CLEANUP_IS_ON) {
        _this._destroyFileHook(instance, options);
      }

      if (typeOf(file) === 'Object' && typeof file.mimetype === 'string' && typeof file.path === 'string') {

        return _this._moveFromTemporary(file, instance).then(function (file) {
          return _this._attachFile(instance, file);
        }).catch(function (err) {
          return _this._validationError(err);
        });
      } else if (typeof file === 'string') {

        var url = file;
        var foldername = instance.getDataValue(_this._FOLDER_KEY);
        var filename = _this._BASEPATH + "/" + foldername + ("/" + nameFromUrl(url));

        return download(url, filename).then(function (file) {
          return _this._attachFile(instance, file);
        }).catch(function (err) {
          return _this._validationError(err);
        });
      } else if (typeOf(file) === 'Null') {
        instance.setDataValue(_this._PATH_ATTRIBUTE_NAME, null);
      }
    };
    ...
    this._destroyFileHook = function (instance, options) {
      ...
      path = _this._fromPublic(path);

      // If sizes are set, file is object, and string
      // otherwise
      if (_this._SIZES) {
        _this._forEachSize(_this._SIZES, function (name, options) {
          var path = pathWithSize(path, name);
    ...
    this.addHooksTo = function (Model) {
      ...
      Model.afterCreate(_this._setFileHook);
      Model.beforeUpdate(_this._setFileHook);
  ...
  _createClass(SequelizeField, [{
    ...
    value: function _validationError(err) {
      ...
      var error = new _sequelize.ValidationError(message, [validationError]);
    ...
    value: function _moveFromTemporary(file, instance) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var instancePath = "" + _this2._BASEPATH.replace(/\/$/, '') + ("/" + instance.getDataValue(_this2._FOLDER_KEY));
    ...
    value: function _processImage(instance, file) {
      ...
      return getSize(file.path).then(function (_ref2) {
        ...
        if (_this3._CROP_IS_ON && crop) {
          var _crop = getDataValue(_this3._CROP_ATTRIBUTE_NAME);
          /** Crop's props are implied to be in percents
           * - we need absolutes
           */
          _crop = {
            width: width * _crop.width,
            height: height * _crop.height,
            x: width * _crop.x,
            y: height * _crop.y
          };
        ...
        return _this3._resizeAll(gmi, file.path);
      });
    }
  }, {
    key: "_attachFile",
    ...
    value: function _attachFile(instance, file) {
      var _this4 = this;

      return new Promise(function (resolve, reject) {
        if (!new RegExp(_this4._MIMETYPE).test(file.mimetype)) {
          ...
          reject(new Error(_this4._WRONG_TYPE_MESSAGE || "Wrong file MIME type: " + ("should be " + mimetype + ", but got " + file.type)));
        }

        var isImage = /image/.test(file.mimetype);

        instance.setDataValue(_this4._PATH_ATTRIBUTE_NAME, _this4._publicPath(file.path));

        instance.setDataValue(_this4._VIRTUAL_ATTRIBUTE_NAME, _this4._publicPath(file.path));

        if (_this4._SIZES && isImage) {
          return _this4._processImage(instance, file).then(function () {
            return resolve();
          }).catch(function (err) {
            return reject(err);
          });
