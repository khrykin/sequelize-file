  ...
  _createClass(SequelizeField, [{
    ...
    value: function _setFile(instance, options, afterCreate) {
      ...
      if (typeOf(file) === 'Object' && typeof file.mimetype === 'string' && typeof file.path === 'string') {
        ...
        return this._moveFromTemporary(file, instance).then(function (file) {
          return _this2._attachFile(instance, file, afterCreate);
        }).catch(function (err) {
          return _this2._validationError(err);
        });
    ...
    value: function _processImage(instance, file) {
      ...
      return getSize(file.path).then(function (_ref2) {
        ...
        if (_this4._CROP_IS_ON && crop) {
          var _crop = getDataValue(_this4._CROP_ATTRIBUTE_NAME);
    ...
    value: function _attachFile(instance, file, afterCreate) {
      var _this5 = this;
      ...
      if (!new RegExp(this._MIMETYPE).test(file.mimetype)) {
        ...
        reject(new Error(this._WRONG_TYPE_MESSAGE || "Wrong file MIME type: " + ("should be " + mimetype + ", but got " + file.type)));
      }
      ...
      if (afterCreate) {
        promise = instance.update(_defineProperty({}, this._PATH_ATTRIBUTE_NAME, this._publicPath(file.path)));
      }
